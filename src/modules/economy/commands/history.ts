...
const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .setDescriptionLocalizations({ fr: 'Voir vos transactions économiques récentes' })
        .addUserOption(opt => 
            opt.setName('user')
               .setNameLocalizations({ fr: 'utilisateur' })
               .setDescription('User to view history for (optional)')
               .setDescriptionLocalizations({ fr: 'Utilisateur dont voir l\'historique (optionnel)' })
               .setRequired(false)
        ),
    ...
    execute: createCommandHandler(async (interaction: ChatInputCommandInteraction, lang: string) => {
        ...
        const history = await EconomyService.getRecentTransactions(targetUser.id, guildId, 10);

        if (history.length === 0) {
            const embed = EmbedUtils.base({
                title: lang === 'fr' ? 'Historique des transactions' : 'Transaction History',
                description: lang === 'fr' ? 'Aucune transaction récente.' : 'No recent transactions.',
                color: '#3498DB',
        ...
        const embed = EmbedUtils.base({
            title: lang === 'fr' ? `Historique de ${targetUser.username}` : `${targetUser.username}'s History`,
            description: desc,
            color: '#3498DB',
            user: interaction.user
        }).setThumbnail(targetUser.displayAvatarURL());
