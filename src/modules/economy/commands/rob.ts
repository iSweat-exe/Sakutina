...
const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .setNameLocalizations({ fr: 'voler' })
        .setDescriptionLocalizations({ fr: 'Voler 1-5% du portefeuille d\'un joueur (cooldown 24h)' })
    ...
    execute: createCommandHandler(async (interaction: ChatInputCommandInteraction, lang: string) => {
        ...
        const embed = EmbedUtils.success(msg, I18nService.translate('common:EMBED_TITLE_ROB_SUCCESS', { lng: lang }), interaction.user);
        await interaction.reply({ embeds: [embed] });
    }),
};
