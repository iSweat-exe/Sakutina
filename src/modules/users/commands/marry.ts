...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('marry')
        .setNameLocalizations({ fr: 'mariage' })
        .setDescription('Marriage system')
        .setDescriptionLocalizations({ fr: 'Système de mariage' })
        .addSubcommand((sub) =>
            sub
                .setName('propose')
                .setNameLocalizations({ fr: 'proposer' })
        ...
        .addSubcommand((sub) =>
            sub
                .setName('divorce')
                .setNameLocalizations({ fr: 'divorcer' })
    ...
    execute: createCommandHandler(
        async (interaction: ChatInputCommandInteraction, lang: string) => {
            ...
            if (subcommand === 'propose') {
                ...
                if (
                    await MarriageService.isMarried(
                        guildId,
                        interaction.user.id
                    )
                ) {
                ...
                if (await MarriageService.isMarried(guildId, target.id)) {
                    const embed = EmbedUtils.error(
                ...
                const marriage = await MarriageService.getMarriage(
                    guildId,
                    target.id
                );
                if (!marriage) {
