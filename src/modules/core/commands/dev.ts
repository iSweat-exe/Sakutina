    ...
    if (subcommand === "eval") {
      ...
      try {
        ...
        if (formattedResult.length > 3900) {
          formattedResult = formattedResult.substring(0, 3900) + "...";
        }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "💽 Memory (RSS)", value: formatBytes(memoryUsage.rss), inline: true },
          { name: "💽 Memory (Heap)", value: `${formatBytes(memoryUsage.heapUsed)} / ${formatBytes(memoryUsage.heapTotal)}`, inline: true },
      ...
      try {
        ...
        await interaction.editReply({ content: I18nService.translate("common:DEV_CLEARDB_SUCCESS", { lng: lang }) });
      } catch (error: any) {
        await interaction.editReply({ content: `❌ Error clearing DB: ${error.message}` });
      }
    }
      ...
      try {
        ...
        await interaction.editReply({ content: `❌ Deploy failed: \`\`\`bash\n${error.message}\n\`\`\`` });
      }
    }
  },
};
