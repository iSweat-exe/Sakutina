...
export class ModerationService {
  ...
  public static async sendLog(
    guild: Guild,
    action: "WARN" | "KICK" | "BAN" | "MUTE" | "UNMUTE" | "SOFTBAN" | "CLEARWARNS",
    target: User,
    moderator: User,
    reason: string,
    extraInfo?: string
    ...
    const channel = guild.channels.cache.get(config.modLogChannel) as TextChannel | undefined;
    if (!channel) return;
    ...
    if (extraInfo) {
      embed.addFields({ name: "Extra Info", value: extraInfo, inline: false });
    }
  ...
  public static async warn(guild: Guild, target: GuildMember, moderator: User, reason: string): Promise<{ autobanned: boolean; warnsCount: number; maxWarns: number }> {
    ...
    // 2. Check total warns
    const userWarns = await db.select().from(warns).where(and(eq(warns.guildId, guild.id), eq(warns.userId, target.id)));
    const config = await GuildConfigService.getGuildSettings(guild.id);
    ...
    // 4. Log the warn
    await this.sendLog(guild, "WARN", target.user, moderator, reason, `Total Warnings: ${userWarns.length}/${maxWarns}`);
  ...
  public static async getWarnings(guildId: string, userId: string) {
    return db.select().from(warns).where(and(eq(warns.guildId, guildId), eq(warns.userId, userId)));
  }

  /**
   * Clear all warnings for a user
   */
  public static async clearWarnings(guild: Guild, target: User, moderator: User, reason: string) {
    const deleted = await db.delete(warns).where(and(eq(warns.guildId, guild.id), eq(warns.userId, target.id))).returning();
    await this.sendLog(guild, "CLEARWARNS", target, moderator, reason, `Cleared ${deleted.length} warnings.`);
